...
task :delete_symlinks do
  ...
  system 'rm ~/.ackrc' if File.exists?(va)
  ...
  puts 'Vim file/folder and bash config files (.bashrc, .bash_profile, .ackrc) deleted!'
end
...
task :create_symlinks => :delete_symlinks do
  ...
  system 'ln -s ' + fullPath + '/vim/ackrc ~/.ackrc'
